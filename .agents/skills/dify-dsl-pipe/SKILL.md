...
# 方式 3：Admin API Key（多 workspace 场景推荐）
npx dify-dsl-pipe export --url <URL> --token-admin <ADMIN_API_KEY> --workspace <WORKSPACE_ID>
```
...
# 导出所有 workspace（默认按 workspace 分目录）
npx dify-dsl-pipe export --url <URL> --token <TOKEN> --all-workspaces
...
# 导入前过滤（按类型/标签/名称）
npx dify-dsl-pipe import --url <URL> --token <TOKEN> --source ./backup --filter "type:workflow,name:客服"
```
...
- `--token-admin` 仅在配合 `--workspace` 时才会自动附加 `X-WORKSPACE-ID`；用于需要按 workspace header 鉴权的场景
- `--all-workspaces` 会遍历并导出全部 workspace；默认命名模板会从 `by-type` 自动升级为 `by-workspace`
...
- `import --filter` 与 export 语法一致：同类型条件 OR，不同类型条件 AND，`name` 为包含匹配
