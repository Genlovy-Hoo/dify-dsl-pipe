...
# 使用 Admin API Key + Workspace（自动附加 X-WORKSPACE-ID）
...
# 导入时可按类型/标签/名称过滤
npx dify-dsl-pipe import --url https://target-dify.com/console/api --token TOKEN \
  --source ./dify-backup --filter "type:workflow,name:客服"
...
> 需要跨 workspace（尤其使用 Admin API Key）时，可使用 `--workspace` 或 `--all-workspaces`。
